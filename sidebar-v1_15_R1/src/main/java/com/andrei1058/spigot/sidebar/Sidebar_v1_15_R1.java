...
public class Sidebar_v1_15_R1 implements com.andrei1058.spigot.sidebar.Sidebar {
    ...
    private ConcurrentHashMap<String, PlayerList_v1_15_R1> teamLists = new ConcurrentHashMap<>();

    private SidebarObjective sidebarObjective;

    public Sidebar_v1_15_R1(@NotNull SidebarLine title, @NotNull Collection<SidebarLine> lines, Collection<PlaceholderProvider> placeholderProvider) {
    ...
    public void playerListCreate(Player player, SidebarLine prefix, SidebarLine suffix, boolean disablePushing) {
        ...
        PlayerList_v1_15_R1 team = new PlayerList_v1_15_R1(this, player, prefix, suffix, disablePushing);
    ...
    public void playerListAddPlaceholders(Player player, PlaceholderProvider[] placeholderProviders) {
        PlayerList_v1_15_R1 list = teamLists.getOrDefault(player.getName(), null);
        if (list == null) return;
    ...
    public void playerListRemove(String teamName) {
        PlayerList_v1_15_R1 list = teamLists.getOrDefault(teamName, null);
        if (list != null) {
    ...
    public void playerListHideNameTag(@NotNull Player player) {
        PlayerList_v1_15_R1 listed = teamLists.get(player.getName());
        if (listed != null) {
            listed.hideNameTag();
        }
    }
    ...
    public void refreshHealth(Player player, int health) {
        ...
        CustomScore_v1_15_R1.sendScore(this, player.getName(), health);
    }
