...
import static net.minecraft.server.v1_12_R1.IScoreboardCriteria.EnumScoreboardHealthDisplay.INTEGER;

class Sidebar_v1_12_R1 implements com.andrei1058.spigot.sidebar.Sidebar {
    ...
    private final ConcurrentHashMap<String, PlayerList_v1_12_R1> teamLists = new ConcurrentHashMap<>();

    public Sidebar_v1_12_R1(@NotNull SidebarLine title, @NotNull Collection<SidebarLine> lines, Collection<PlaceholderProvider> placeholderProvider) {
    ...
    public void addPlaceholder(PlaceholderProvider placeholderProvider) {
        ...
        for (Sidebar_v1_12_R1.ScoreLine sbLine : lines) {
            if (!sbLine.text.isHasPlaceholders()) {
                if (sbLine.text instanceof SidebarLineAnimated) {
                    for (String line : ((SidebarLineAnimated) sbLine.text).getLines()) {
                        if (line.contains(placeholderProvider.getPlaceholder())) {
                            sbLine.text.setHasPlaceholders(true);
                            break;
                        }
                    }
                } else if (sbLine.text.getLine().contains(placeholderProvider.getPlaceholder())) {
                    sbLine.text.setHasPlaceholders(true);
                }
            }
        }
    ...
    public void playerListHideNameTag(@NotNull Player player) {
        PlayerList_v1_12_R1 listed = teamLists.get(player.getName());
        if (listed != null) {
            listed.hideNameTag();
        }
    }
    ...
    public void setLine(SidebarLine sidebarLine, int line) {
        if (line >= 0 && line < this.lines.size()) {
            ...
            for (PlaceholderProvider provider : placeholderProviders) {
                if (sidebarLine.getLine().contains(provider.getPlaceholder())) {
    ...
    public void apply(Player player) {
        ...
        if (healthObjective != null) {
            ...
            for (PlayerList_v1_12_R1 team : teamLists.values()) {
    ...
    public void refreshAnimatedLines() {
        for (Sidebar_v1_12_R1.ScoreLine sbLine : lines) {
            if (sbLine.text instanceof SidebarLineAnimated) {
                if (sbLine.text.isHasPlaceholders()) {
                    String content = sbLine.text.getLine();
                    for (PlaceholderProvider pp : this.placeholderProviders) {
                        if (content.contains(pp.getPlaceholder())) {
                            content = content.replace(pp.getPlaceholder(), pp.getReplacement());
                        }
                    }
                    sbLine.setContent(content);
                } else {
                    sbLine.setContent(sbLine.text.getLine());
                }
                sbLine.sendUpdate();
            }
        }
    ...
    public void playerListAddPlaceholders(@NotNull Player player, PlaceholderProvider... placeholderProviders) {
        PlayerList_v1_12_R1 list = teamLists.getOrDefault(player.getName(), null);
        if (list == null) return;
    ...
    public void playerListClear() {
        for (PlayerList_v1_12_R1 team : teamLists.values()) {
            for (PlayerConnection playerConnection : players) {
                team.sendRemove(playerConnection);
            }
        }
        teamLists.clear();
    }
    ...
    public void hidePlayersHealth() {
        if (healthObjective != null) {
            for (PlayerConnection playerConnection : players) {
                healthObjective.sendRemove(playerConnection);
            }
    ...
    public void refreshHealth(@NotNull Player player, int health) {
        ...
        CustomScore_v1_12_R1.sendScore(this, player.getName(), health);
    }
    ...
    private static void scoreOffsetIncrease(@NotNull Collection<ScoreLine> lineCollections) {
        for (Sidebar_v1_12_R1.ScoreLine line : lineCollections) {
